...
export default defineEventHandler(async (event) => {
  ...
  try {
    const updatedListing = await updateRokListingStatus(event, listingId, 'sold');
    ...
    if (updatedListing.userId) {
      try {
        await createNotification(
          ...
          undefined,
          'sold',
          'Rise of Kingdoms Listing Marked as Sold',
          'Your Rise of Kingdoms listing has been marked as sold.'
        );
      } catch {}
    }
    ...
    throw createError({ statusCode: 500, statusMessage: "Failed to mark ROK listing as sold. Please try again." });
  }
});
