...
export default defineEventHandler(async (event) => {
  ...
  try {
    const updatedListing = await updateRokListingStatus(event, listingId, 'approved');
    ...
    if (updatedListing.userId) {
      try {
        await createNotification(
          ...
          undefined,
          'approved',
          'Rise of Kingdoms Listing Approved',
          'Your Rise of Kingdoms listing has been approved and is now visible to buyers.'
        );
      } catch {}
    }
    ...
    throw createError({ statusCode: 500, statusMessage: "Failed to approve ROK listing. Please try again." });
  }
});
